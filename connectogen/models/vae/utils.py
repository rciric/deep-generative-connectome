...
def poly_to_mask(polygon, width, height):
    ...
    # print(np.column_stack(np.where(np.array(img) == 1)))
    mask = np.array(img)
    mask = np.array(mask, dtype=bool)
    # # http://stackoverflow.com/a/3732128/1410871
    # img = Image.new(mode='L', size=(width, height), color=0)
    # ImageDraw.Draw(img).polygon(xy=polygon, outline=1, fill=None)
...
def slice_order(path):
    ...
    ct_file_list = [f for f in os.listdir(path) if f.startswith('CT') and f.endswith('.dcm')]
    for s in ct_file_list:
        try:

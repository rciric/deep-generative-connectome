...
class DataGenerator(object):
    ...
    def generate(self, list_fileinfo_sample=[], list_IDs=[], ext_data='.npz'):
        ...
        while 1:
            ...
            for i in range(imax):
                # Find list of IDs
                list_IDs_temp = [list_IDs[k]
                                 for k in indexes[i*self.batch_size:(i+1)*self.batch_size]]
                self.logout(
                    level=3, content='index in current batch:{0}'.format(list_IDs_temp))
                # Generate data
                X, Y = self.__data_generation(
                    list_fileinfo_sample, list_IDs_temp, ext_data)
    ...
    def __data_generation(self, list_filepath_sample, list_IDs_temp, ext_data='h5'):
        ...
        # generate data
        for index_in_batch, ID in enumerate(list_IDs_temp):
            # get input and output
            filepaths_input, filepath_output, index_slice, scales_mean = ID
            num_inputs = len(filepaths_input)
            data_inputs = []
            #print(filepaths_input)

            # load inputs
            for index_input in range(num_inputs):
            ...
            data_full = self.__get_data_from_h5(filepath_output, 
                                                ...
                                                axis_slice=self.axis_slice, 
                                                augment_25d=self.dim_25d)
            #print([x.shape for x in data_inputs])

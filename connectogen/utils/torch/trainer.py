...
class Trainer(object):
    ...
    def __init__(self,
        ...
        # TODO
        # add this back after implementing dataset and test/train split
        # for ConnectomeDataLoader
        """
...
class ConnectomeTrainer(Trainer):
    """Additional trainer functions for use when learning connectome data.
    """

    def squareform(self, batch):
        """Convert a batch of edgewise feature vectors into matrices or
        matrices into feature vectors.

        WARNING: THIS IS NOT A DIFFERENTIABLE OPERATION.
        """
        return torch.stack([
            torch.Tensor(squareform(i.squeeze())) for i in batch.detach().numpy()
        ])

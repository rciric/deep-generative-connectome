# niworkflows on sherlock isn't up to date, and docker is banned on sherlock,
# so we're S.O.L when it comes to trying to build a confound model on sherlock.

# unless we do this moronic thing, of course.

# so this includes literally the entire code base that I wrote for confound
# modelling in niworkflows lol.
...
rps = 'X + Y + Z + RotX + RotY + RotZ'
param36 = '(dd1({} + WhiteMatter + CSF + GlobalSignal))^^2'.format(rps)
...
class SpikeRegressors(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        spike_criteria = {
            'FramewiseDisplacement': ('>', self.inputs.fd_thresh),
            'stdDVARS': ('>', self.inputs.dvars_thresh)
        }
...
def get_parser():
    ...
    parser.add_argument('--out', '--out',
                        action='store', help='output base')
    parser.add_argument('--confs', '--confs',
                        action='store', help='confound matrix')
    return parser
...
def main():
    ...
    regs_with_spikes = '{}_desc-spikes_regressors.tsv'.format(opts.out)
    spks = SpikeRegressors(
        confounds_file=opts.confs,
        fd_thresh=0.5,
        dvars_thresh=1.5,
        concatenate=False,
        output_format='spikes',
        output_file=regs_with_spikes
    )
    print(spks.run().outputs.confounds_file)

    proc = ExpandModel(
        confounds_file=opts.confs,
        model_formula=param36,
        output_file='{}_desc-param36_regressors.tsv'.format(opts.out)
    )
    print(proc.run().outputs.confounds_file)

    tmask = SpikeRegressors(
        confounds_file=opts.confs,
        fd_thresh=0.5,
        dvars_thresh=1.5,
        concatenate=False,
        output_format='mask',
        output_file='{}_desc-temporal_mask.tsv'.format(opts.out)
    )
    print(tmask.run().outputs.confounds_file)
